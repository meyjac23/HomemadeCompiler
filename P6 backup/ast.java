...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() {
        ...
        TableSym mainSym = symTab.lookupLocal("main");
...
class DeclListNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable symTab, FnSym fnSym) {
        nameAnalysis(symTab, symTab, fnSym);
    }

    public void nameAnalysis(SymTable symTab, SymTable globalTab, FnSym fnSym) {
        int offset = 0;
...
class FnBodyNode extends ASTnode {
     ...
     */
    public void nameAnalysis(SymTable symTab, FnSym fnSym) {
        myDeclList.nameAnalysis(symTab, fnSym);
...
class VarDeclNode extends DeclNode {
    ...
    public TableSym nameAnalysis(SymTable symTab, SymTable globalTab) {
        ...
        if(sym.isGlobal){
            ...
            sym.setOffset(symTab.getOffset());
        }
        symTab.setOffset(symTab.getOffset() - 4);
        return sym;
    }
...
class FnDeclNode extends DeclNode {
    ...
    public void codegen() {
        ...
        if (myId.name().compareTo("main") == 0) {
            Codegen.generate(".globl main");
            Codegen.genLabel("main");
        }
        else {
            String label = Codegen.nextLabel();
            Codegen.genLabel("_" + label);
        }
        // Function entry
        int locals = ((FnSym)(myId.sym())).getSizeLocals();
        Codegen.genPush(Codegen.RA);
        Codegen.genPush(Codegen.FP);
        Codegen.generateWithComment("addu", "Set $fp", Codegen.FP, Codegen.SP, "8");
        Codegen.generate("subu", Codegen.SP, Codegen.SP, String.valueOf(locals));
        // Function Body
        myBody.codegen();
        // Function Exit
    ...
    public TableSym nameAnalysis(SymTable symTab) {
        ...
        myBody.nameAnalysis(symTab, (FnSym) myId.sym()); // process the function body
...
class StringLitNode extends ExpNode {
    ...
    public void codegen() {
        // store string value in .data section with label
...
class IdNode extends ExpNode {
    ...
    public void genAddr() {
        if (mySym.isGlobal) {
            ...
            // Load offset into T0, subtract unsigned from $fp to get addr
            Codegen.generate("li", Codegen.T0, String.valueOf(offset));
            Codegen.generate("subu", Codegen.T1, Codegen.FP, Codegen.T0);
...
class DotAccessExpNode extends ExpNode {
    ...
    private IdNode myId;
    private TableSym mySym;          // link to Sym for struct type
...
class AssignNode extends ExpNode {
    ...
    public void codegen() {
        // Can assume that Lhs will be IdNode
        ...
        if (myExp instanceof IdNode) {
            ...
            // Assuming that myExp will push it's literal value to the stack
            myExp.codegen();
            Codegen.genPop(Codegen.T0);
            myId.genAddr();
            Codegen.genPop(Codegen.T1);
